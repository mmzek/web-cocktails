...
export default function App() {
  ...
  useEffect(()=>{
    ...
    return () => {animation.destroy(); clearTimeout(timer)} ;
  }
    ,[])
  ...
  return (
    <Routes>
        <Route
          ...
          element={
            <>
              {showAnimation ? (<div className="fixed inset-0 flex items-center justify-center bg-black">
 <div ref={container} className='w-1/4'/>
</div>) : (<> <Navbar />
              <HeroSection />
              <CocktailsList categories={categories} cocktailsByCategory={cocktailsByCategory} loading={loading}/> </> )}
            </>
          }
        />
