...
export default function Favourites({ favouritesTable, cocktailsByCategory }: FavouritesProps) {
  ...
  return (
    <div>
      ...
      {favouriteCocktails.length > 0 ? (
        <div> <h1 className="text-gray-900 text-6xl font-medium md:mx-30 sm:mx-20 my-10 animation-appear">
        Favourites
      </h1><div className="m-30 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3  gap-4 p-4">
        {favouriteCocktails.map(cocktail => (
          <div key={cocktail.id} className=" bg-gray-100 rounded-2xl " onClick={() => navigate(`/description/${cocktail.id}`)}>
        ...
        <p className="text-gray-900 text-6xl font-medium text-center col-span-2">No favourites yet!</p>
      )}
    </div>
